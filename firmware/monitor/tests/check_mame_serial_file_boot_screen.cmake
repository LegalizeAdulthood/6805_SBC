...
file(WRITE "${_stage_dir}/cfg/m6805sbc.cfg"
    "<?xml version=\"1.0\"?>\r\n"
    "<!-- This file is generated by the monitor serial-file boot test. -->\r\n"
...
file(WRITE "${_serial_script}"
    ...
    "        if idle_frames < 180 and frames < 420 then return end\r\n"
    "        print(string.format(\"SERIAL_FILE_BOOT_SCREEN FRAMES=%d\", frames))\r\n"
    "        manager.machine:exit()\r\n"
    "        return\r\n"
    "    end\r\n"
    "end, \"serial_file_boot_screen\")\r\n"
)
...
execute_process(
    COMMAND
        ...
        -autoboot_delay 0
        -autoboot_script serial_file_boot_screen.lua
        -seconds_to_run 7
...
string(REGEX MATCH "SERIAL_FILE_BOOT_SCREEN FRAMES=([0-9]+)" _output_match "${_mame_output}")
if(NOT _output_match)
    message(FATAL_ERROR "MAME output did not report SERIAL_FILE_BOOT_SCREEN\n${_mame_output}")
endif()

if(NOT EXISTS "${_serial_output_file}")
    message(FATAL_ERROR "Serial output file was not created: ${_serial_output_file}\n${_mame_output}")
endif()

file(READ "${_serial_output_file}" _actual_bytes HEX)
string(TOUPPER "${_actual_bytes}" _actual_bytes)
string(LENGTH "${_actual_bytes}" _actual_hex_length)
math(EXPR _actual_count "${_actual_hex_length} / 2")

if(NOT _actual_count EQUAL _expected_count)
    message(FATAL_ERROR "Expected ${_expected_count} serial boot bytes, got ${_actual_count}\n${_mame_output}")
endif()

if(NOT _actual_bytes STREQUAL "${_expected_bytes}")
    message(FATAL_ERROR "Expected serial boot bytes ${_expected_bytes}, got ${_actual_bytes}\n${_mame_output}")
endif()
